...
class AssessmentAggregatorFunction(config: AssessmentAggregatorConfig,
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    dataCache = new DataCache(config, new RedisConnect(config,Option(config.redisHost),Option(config.redisPort)), config.relationCacheNode, List())
    dataCache.init()
    contentCache = new DataCache(config, new RedisConnect(config,Option(config.redisHost),Option(config.redisPort)), config.contentCacheNode, List())
    contentCache.init()
