...
import org.sunbird.job.domain.reader.Events
...
class CoreTestSpec extends BaseSpec with Matchers with MockitoSugar {
  ...
  "RedisConnect functionality" should "be able to connect to redis" in {
    val redisConnection = new RedisConnect(baseConfig,Option(baseConfig.redisHost), Option(baseConfig.redisPort))
  ...
  "DataCache hgetAllWithRetry function" should "be able to retrieve the map data from Redis" in {
    ...
    val dataCache = new DataCache(baseConfig, new RedisConnect(baseConfig,Option(baseConfig.redisHost), Option(baseConfig.redisPort)), 2, deviceFields)
    dataCache.init()
