...
class AssessmentAggregatorTaskTestSpec extends BaseTestSpec {
  ...
  ignore should "Skip the missing records from the event" in {
  ...
  def setupRedisTestData() {
    val redisConnect = new RedisConnect(assessmentConfig,Option(assessmentConfig.redisHost), Option(assessmentConfig.redisPort) )
